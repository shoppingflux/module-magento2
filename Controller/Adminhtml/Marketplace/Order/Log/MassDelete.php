...
class MassDelete extends LogAction
{
    const ADMIN_RESOURCE = 'ShoppingFeed_Manager::marketplace_order_log_delete';
    ...
    public function execute()
    {
        try {
            ...
            $unknownCount = $logCollection->count() - $deletedCount - $errorCount;

            if ($deletedCount > 0) {
                $this->messageManager->addSuccessMessage(
                    __('%1 order logs have been deleted.', $deletedCount)
                );
            }

            if ($errorCount > 0) {
                $this->messageManager->addErrorMessage(
                    __('%1 order logs could not be deleted (see error log for details).', $errorCount)
                );
            }
            ...
            $this->messageManager->addExceptionMessage(
                $e,
                __('An error occurred while deleting the order logs.')
            );
        }
